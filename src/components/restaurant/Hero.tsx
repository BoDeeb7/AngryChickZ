    ...
    <section className="relative min-h-screen flex items-center pt-32 pb-40 overflow-hidden ambient-glow mesh-transition">
      ...
      <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center relative z-10">
        <div className="space-y-10 animate-in fade-in slide-in-from-left-12 duration-1000">
          ...
          <h1 className="text-6xl lg:text-[90px] font-black leading-[0.95] tracking-tighter text-foreground uppercase italic">
          ...
          <div className="flex flex-wrap gap-6">
            <Button 
              size="lg" 
              className="h-24 px-12 rounded-[2.5rem] bg-primary hover:bg-primary/90 text-white text-2xl font-black transition-all shadow-[0_20px_40px_-10px_rgba(225,29,72,0.5)] group uppercase italic" 
            ...
            <Button 
              size="lg" 
              variant="outline" 
              className="h-24 px-12 rounded-[2.5rem] text-2xl font-black border-primary/10 hover:bg-primary/5 uppercase italic backdrop-blur-sm" 
              onClick={scrollToMenu}
            >
              Explore Menu
            </Button>
              ...
              <div className="flex -space-x-4">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="h-12 w-12 rounded-full border-2 border-background bg-muted overflow-hidden">
                    <Image src={`https://picsum.photos/seed/${i + 80}/48/48`} alt="user" width={48} height={48} />
                  </div>
                ))}
              </div>
              <div>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map(i => <Star key={i} className={`h-3.5 w-3.5 ${i <= Math.round(Number(averageRating)) ? 'fill-secondary text-secondary' : 'text-foreground/10'}`} />)}
                </div>
                <p className="text-[11px] font-black text-foreground/40 uppercase tracking-widest mt-2">
        ...
        <div className="relative flex justify-center items-center animate-in fade-in zoom-in-90 duration-1000 delay-300">
          ...
          <div className="relative w-full max-w-[600px] aspect-square transition-transform duration-1000 hover:rotate-2">
            <Image 
          ...
          <div className="absolute -top-10 -right-5 glass-card p-10 rounded-[3rem] rotate-12 border-primary/10 shadow-2xl">
            <p className="text-[11px] font-black uppercase tracking-[0.4em] text-primary mb-3">Elite Signature</p>
