      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-20 mb-40">
          <div className="space-y-12">
            ...
            <p className="text-white/30 leading-relaxed font-bold text-[10px] uppercase tracking-[0.2em]">
              Gourmet fried chicken obsession. Established 2024. Level 5 heat.
            </p>
            <div className="flex gap-4">
              {[Instagram, Facebook, Twitter].map((Icon, idx) => (
                <Link key={idx} href="#" className="h-12 w-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center hover:bg-primary transition-all duration-500 group">
                  <Icon className="h-5 w-5 text-white/30 group-hover:text-white" />
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white/[0.03] p-10 rounded-[3rem] border border-white/5 shadow-2xl">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-5">
              <div className="flex gap-4">
                <Input 
                  ...
                  className="h-14 bg-white/5 border-white/5 rounded-2xl text-[10px] outline-none font-black tracking-widest" 
                />
                <div className="flex items-center gap-2 bg-white/5 px-4 rounded-2xl border border-white/5">
                  {[1,2,3,4,5].map(i => (
                    <Star 
                      key={i} 
                      className={`h-4 w-4 cursor-pointer transition-colors ${i <= review.rating ? 'fill-secondary text-secondary' : 'text-white/10 hover:text-white/40'}`} 
              ...
              <Textarea 
                ...
                className="h-24 bg-white/5 border-white/5 rounded-2xl text-[10px] font-black tracking-widest pt-5" 
              />
              <Button disabled={isSubmitting} className="w-full h-14 bg-primary hover:bg-primary/90 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.3em]">
        ...
        <div className="pt-24 border-t border-white/5 flex flex-col lg:flex-row items-center justify-between gap-16">
          <p className="text-[9px] text-white/10 font-black uppercase tracking-[0.5em]">© 2024 ANGRY CHICKZ ELITE. ALL RIGHTS RESERVED.</p>
          
          <button onClick={scrollToTop} className="h-14 w-14 rounded-full bg-white/5 border border-white/5 flex items-center justify-center hover:bg-primary transition-all duration-700 group">
            <ArrowUp className="h-5 w-5 text-white/10 group-hover:text-white" />
          </button>

          <div className="flex flex-col items-center lg:items-end group">
            <div className="flex items-center gap-3 mb-4 opacity-40">
               <Sparkles className="h-3 w-3 text-secondary animate-pulse" />
               <span className="text-[9px] font-black uppercase tracking-[0.4em] text-white">Elite Signature Release</span>
            </div>
            <div className="glass-card px-8 py-4 rounded-[2rem] border-white/5 bg-white/[0.02] shadow-[0_0_40px_rgba(225,29,72,0.15)] group-hover:scale-105 transition-transform duration-700">
